...
function smarty_function_paginate_middle($params, &$smarty) {
    ...
    while($_item <= $_total) {
        ...
        if($_item != $_curr_item) {
            ...
            $_ret .= $_link_prefix . '<li class="page-item"><a class="page-link" href="' . str_replace('&', '&amp;', $_this_url) . '"' . $_attrs . '>' . $_text . '</a></li>' . $_link_suffix;
        } else {
            $_ret .= "<li class='page-item'><a class='page-link active' >" . $_link_prefix . $_text . $_link_suffix ."</a></li>";
        }
        $_item += $_limit;
        $_page++;
        $_display_pages++;
