...
class simulasiHitungIKLController extends Front
{
    ...
    public function hitungNasional()
    {
      $uid_indeks = $this->params('x');
            ...
            if ($dataIndeks['total']) {
                ...
                // $this->debug->show($nilai_indeks_tmp);
    ...
    public function test(){
      $data = $this->iktl->cnIndeks2025(2026,1,1, true);
        ...
        if ($dataIndeks['total']) {
            ...
            if ($jenis_indeks == 1) {
                if($dataIndeks["tahun"] >= 2025){
                  $dataIndeksCn = $this->iktl->cnIndeks2025($dataIndeks["tahun"],$dataIndeks['uid_provinsi'],$dataIndeks['uid_kabkota'],true);
                  ...
                  $getDkkKabkota = $this->tables->query("SELECT * FROM nilai_dkk WHERE deleted = 0 AND peruntukan = 1 AND uid_provinsi=".$dataIndeks['uid_provinsi']." AND uid_kabkota=".$dataIndeks['uid_kabkota']." AND tahun=".$dataIndeks['tahun']);
                  $json_data['dkk'] = ($getDkkKabkota['data'][0]['nilai'] ? $getDkkKabkota['data'][0]['nilai'] : 0);
                ...
                if($dataIndeks["tahun"] >= 2025){
                  ...
                  $dataIndeksCn = $this->iktl->cnIndeks2025($dataIndeks["tahun"],$dataIndeks['uid_provinsi'],0,true);
                  $json_data['tl'] = $dataIndeksCn['itl'];
                  $json_data['iktl'] = $dataIndeksCn['iktl'];
                  $json_data['dkk'] = 0;
                  $json_data['tl_dkk'] = 0;
