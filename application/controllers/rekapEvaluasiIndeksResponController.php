...
class rekapEvaluasiIndeksResponController extends Front
{
    ...
    private function dataTable()
    {
      ...
      $sql = 'SELECT MIN(a.indeks_uid) as min_index, GROUP_CONCAT(a.indeks_uid SEPARATOR ",") indeks_gabung, a.users_uid, a.tahun, MIN(a.tipe) as min_tipe, GROUP_CONCAT(a.tipe SEPARATOR ",") AS tipe_gabung,
                     b.name, b.role_user, b.uid_provinsi, b.uid_kabkota, c.kd_regional, c.nama_propinsi, d.nama_kabkot
              FROM ir_parent a
      ...
      foreach ($dataParent['data'] as $key => $val) {
        if($val['uid_provinsi'] == 0 && $val['uid_kabkota'] == 0){
          ...
          //PANTAI BERSIH
          $data2['data'][$key] = $val;
          $data2['data']['total'] = $dataParent['total'];
          $sql = 'SELECT *
          FROM indeks_respon
          ...
          // PROGRAM KALI BERSIH
          $data3['data'][$key] = $val;
          $data3['data']['total'] = $dataParent['total'];
          $sql = 'SELECT *
          FROM indeks_respon
          ...
          // INDONESIA HIJAU
          $data4['data'][$key] = $val;
          $data4['data']['total'] = $dataParent['total'];
          $sql = 'SELECT *
          FROM indeks_respon
          ...
          // GAMBUT LESTARI
          $data5['data'][$key] = $val;
          $data5['data']['total'] = $dataParent['total'];
          $sql = 'SELECT *
          FROM indeks_respon
        ...
        $data2['data'] = array_values($data2['data']);
        $data3['data'] = array_values($data3['data']);
        $data4['data'] = array_values($data4['data']);
        $data5['data'] = array_values($data5['data']);

        // $this->debug->show($data5);
      }
