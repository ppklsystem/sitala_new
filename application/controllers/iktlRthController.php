 ...
 * desc : controller for ikl rth
 */
class iktlRthController extends Front {
	public function init() {
		...
		$this -> view -> assign("primaryKey", "pelaporan_iktl_rth_uid");
		$this -> viewName = "pelaporan_iktl_rth";
		$this -> primaryKey = "pelaporan_iktl_rth_uid";
    ...
    $this -> view -> assign("message", $message);
		...
		$this -> view -> assign("title", 'RTH Indek Kualitas Lahan ');
  ...
  private function postData(){
    ...
    if(isset($post['submit'])){
      if($post['pelaporan_iktl_uid'] > 0){
        ...
        if($files) {
          ...
          foreach ($data as $key => $val) {
            if($val[1]){
              ...
              $dataDetail[$idxData]['jenis_rth'] = trim($refJenisRth[$idxJenisRth]['idx']);
              $dataDetail[$idxData]['luas_rth'] = ($val[2]? "'".trim(str_replace("'","''", $val[2]))."'" : "''");
							// $dataDetail[$idxData]['nama_rth'] = ($val[3]? "'".trim(str_replace("'","''", $val[3]))."'" : "''");
              $dataDetail[$idxData]['nama_rth'] = ($val[3]? "'".trim($this->replaceinput($val[3]))."'" : "''");
              ...
              $arrayPush = array(
                ...
                $dataDetail[$idxData]['jenis_rth'],
                $dataDetail[$idxData]['luas_rth'],
                $dataDetail[$idxData]['nama_rth'],
        ...
        if(count($onUpdateMultiple) > 0){
            ...
            $sql = "INSERT INTO pelaporan_iktl_rth (deleted,hidden,crdate,chdate,pelaporan_iktl_uid,jenis_rth,luas_rth,nama_rth,nama_desa,nama_kecamatan,koordinat_lintang,koordinat_bujur,sumber_dana,nomor_sk,keterangan,verify) VALUES ".$valUpdate."";
            // $this->debug->show($sql);
		...
		if(isset($post['copy'])){
			if(isset($post['pelaporan_iktl_uid']) && isset($post['periode'])){
				$data = $this->db->fetch("SELECT * FROM pelaporan_iktl_rth WHERE deleted = 0 AND pelaporan_iktl_uid=".$post['periode']);
				if($data['total'] > 0){
					$onUpdateMultiple = [];
					foreach ($data['data'] as $key => $value) {
						$dataDetail[$key]['jenis_rth'] = trim($value['jenis_rth']);
						$dataDetail[$key]['luas_rth'] = ($value['luas_rth']  ? "'".trim($this->replaceinput($value['luas_rth']))."'" : "''");
						$dataDetail[$key]['nama_rth'] = ($value['nama_rth']  ? "'".trim($this->replaceinput($value['nama_rth']))."'" : "''");
						...
						$arrayPush = array(
							...
							$post['pelaporan_iktl_uid'],
							$dataDetail[$key]['jenis_rth'],
							$dataDetail[$key]['luas_rth'],
							$dataDetail[$key]['nama_rth'],
					...
					if(count($onUpdateMultiple) > 0){
	            $valUpdate = implode(",", $onUpdateMultiple);
	            $sql = "INSERT INTO pelaporan_iktl_rth (deleted,hidden,crdate,chdate,pelaporan_iktl_uid,jenis_rth,luas_rth,nama_rth,nama_desa,nama_kecamatan,koordinat_lintang,koordinat_bujur,sumber_dana,nomor_sk,keterangan,catatan,verify,from_data) VALUES ".$valUpdate."";
	            $return = $this->db->query($sql);
	            if($return){
								$this->countData($post['pelaporan_iktl_uid']);
	              return "Success, Data berhasil disimpan";
    ...
    if (isset($post['x']) && is_numeric($post['x']) ) {
        $this -> tables -> set("pelaporan_iktl_rth", "pelaporan_iktl_uid");
				$post['form']['pelaporan_iktl_uid'] = $post['x'];
				$post['form']['verify'] = 2;
				$post['submit'] = TRUE;
        if ($this -> tables -> post($post)) {
						$this->countData($post['x']);
            echo json_encode(array('statusCode' => 200, 'message' => $this -> message -> save('success'), 'href'=>'iktlRth/index/x/'.base64_encode($this->params("x"))."/verify/1"));
        } else {
  ...
  public function deletedDataAll(){
    ...
    if (isset($post['x'])) {
        ...
        if ($this -> tables -> softDelete($post['x'])) {
					$this->countData($post['x']);
            echo json_encode(array('statusCode' => 200, 'message' => $this -> message -> delete('success'), 'href'=>'iktlRth/index/x/'.base64_encode($this->params("x"))));
        } else {
  ...
  public function deletedData()
  {
      $post = $this -> post();
      if (isset($post['x'])) {
          $this -> tables -> set("pelaporan_iktl_rth", "pelaporan_iktl_rth_uid");
          if ($this -> tables -> softDelete($post['x'])) {
              echo json_encode(array('statusCode' => 200, 'message' => $this -> message -> delete('success'), 'href'=>'iktlRth/index/x/'.base64_encode($this->params("x"))));
          } else {
              echo json_encode(array('statusCode' => 400, 'message' => $this -> message -> delete('failed')));
	...
	public function countData($idPelaporanByFunction = null){
		// dimatikan karena pelaporan 2025 tidak digunakan
		// pelaporan 2025 dihitung tidak berdasarka verifikasi dan dihitung untuk semua

		$idPelaporan = ($idPelaporanByFunction?$idPelaporanByFunction:$this->params("x"));
		if($idPelaporan>0){
			$totalRth = $this->db->query("SELECT SUM(luas_rth) AS total FROM pelaporan_iktl_rth WHERE deleted = 0 AND pelaporan_iktl_uid=".$idPelaporan);
			// $totalLipi = $this->db->query("SELECT SUM(luas_rth) AS total FROM pelaporan_iktl_rth WHERE deleted = 0 AND verify = 1 AND jenis_rth = 1 AND pelaporan_iktl_uid=".$idPelaporan);
			// $totalKehati = $this->db->query("SELECT SUM(luas_rth) AS total FROM pelaporan_iktl_rth WHERE deleted = 0 AND verify = 1 AND jenis_rth = 2 AND pelaporan_iktl_uid=".$idPelaporan);

			$result['uid_pelaporan_iktl'] = $idPelaporan;
			$result['rth'] = ($totalRth->fields['total']?$totalRth->fields['total']:0);
  ...
  private function getData(){
    ...
    $properties['data']	= ['luas_rth','nama_rth','nama_desa','nama_kecamatan','koordinat_lintang','koordinat_bujur','nomor_sk','keterangan','catatan'];
    ...
    if(isset($post['search'])){
      ...
      if($post['form']['jenis_rth']){
        $w .= ' AND jenis_rth='.$post['form']['jenis_rth'];
      }
	...
	private function exportExcel($data){
		...
		header('Content-Disposition: attachment; filename="IKTL_RTH_'.time().'.xls"');
		$html = $this->view->fetch('parts/contents/iktlRth/index/excel.html');
		echo $html;
		die();
	}
  ...
  public function updateData(){
    ...
    if($post['idx'] > 0 && $post['field'] ){
      ...
      $update['submit'] = TRUE;
      $this->tables->set('pelaporan_iktl_rth', $this->primaryKey);
      $updateStatus = $this->tables->post($update);
      if($updateStatus){
				if($post['field'] == 'verify'){
					$dataId = $this->db->fetch("SELECT pelaporan_iktl_uid FROM pelaporan_iktl_rth WHERE pelaporan_iktl_rth_uid = ".$post['idx'])['data'][0];
  ...
  // public function syncDataRth(){
  //   $data = $this->db->query("SELECT * FROM pelaporan_iktl WHERE deleted = 0 AND data_rth != ''");
  //   $dataTmp = [];
  //   $onUpdateMultiple = [];
  //   $created = time();
  //   $chdate = time();
  //   foreach ($data as $key => $value) {
  //     if(is_array(json_decode($value['data_rth'],TRUE))){
  //       $dataRth = ($value['data_rth_verifikasi'] ? json_decode($value['data_rth_verifikasi'],TRUE) : json_decode($value['data_rth'],TRUE));
  //       foreach ($dataRth['data_rth'] as $ki => $vi) {
  //         $arrayPush = array(
  //           0,0,$created,$chdate,
  //           $value['uid_pelaporan_iktl'],
  //           ($vi['jenis_rth']? trim($vi['jenis_rth']) : 0),
  //           ($vi['luas_rth']? "'".trim(str_replace("'","''", $vi['luas_rth']))."'" : "''"),
  //           ($vi['nama_rth']? "'".trim(str_replace("'","''", $vi['nama_rth']))."'" : "''"),
  ...
  //   $this->debug->show($onUpdateMultiple);
  //   $valUpdate = implode(",", $onUpdateMultiple);
  //   $sql = "INSERT INTO pelaporan_iktl_rth (deleted,hidden,crdate,chdate,pelaporan_iktl_uid,jenis_rth,luas_rth,nama_rth,nama_desa,nama_kecamatan,koordinat_lintang,koordinat_bujur,sumber_dana,nomor_sk,keterangan,catatan,verify) VALUES ".$valUpdate."";
  //   // $this->debug->show($sql);
  //   $return = $this->db->query($sql);
