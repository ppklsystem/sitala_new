...
class simulasiHitungIKUController extends Front
{
    ...
    public function test(){
      // die("hi");
      // $data = $this->iku->cnIndeks2025(2025,1,0,true);
      // $data = $this->iku->cnIndeks2025(2025,13,168,true);
      // $data = $this->iku->cnIndeks2025(2025,0,0,true);
      ...
      $uid_indeks = $this->params('x');
            ...
            if ($dataIndeks['total']) {
                ...
                echo json_encode(array(
                        ...
                        "message"=>"success",
                        "data"=>$nilai_indeks,
                        // "data_ina"=>$nilai_indeks_ina,
                        // "data_no_bobot"=>$nilai_indeks_no_bobot,
                    ));
            } else {
    ...
    {//function for counting data pelaporan
      ...
      if ($jenis_indeks == 1) {
          ...
          if ($avgData['total']) {
              ...
              $cnIndeks['rpp']['no2'] = (array_sum($nilaiParam['no2']) > 0 ? array_sum($nilaiParam['no2']) / count($nilaiParam['no2']) : "");
              $cnIndeks['rpp']['so2'] = (array_sum($nilaiParam['so2']) > 0 ? array_sum($nilaiParam['so2']) / count($nilaiParam['so2']) : "");
              $cnIndeks['rpp']['pm25'] = (array_sum($nilaiParam['pm25']) > 0 ? array_sum($nilaiParam['pm25']) / count($nilaiParam['pm25']) : "");

              $_cnIndeks = $this->_cnIndeks($cnIndeks['rpp'], $dataIndeks['tahun']);

              $cnIndeks = array_merge($cnIndeks, $_cnIndeks);

              $cnIndeks2025 = $this->iku->cnIndeks2025($dataIndeks['tahun'],$dataIndeks['uid_provinsi'],$dataIndeks['uid_kabkota'],true);
              $cnIndeks["indeksIkuInaTitik"] = $cnIndeks2025["titik"];
              $cnIndeks["indeksIkuInaDetail"] = $cnIndeks2025["kabkota"];
              $cnIndeks["indeksIkuIna"] = $cnIndeks2025["kabkota"]["iku"];
              $cnIndeks["rataanIndeksIna"] = $cnIndeks2025["kabkota"]["indeks_rataan"];
              $cnIndeks["all"] = $cnIndeks2025;
      ...
      $uid_indeks = $this->params('x');
        ...
        if ($dataIndeks['total']) {
            ...
            if ($dataIndeksKabkota['total']) {
                ...
                $cnIndeks2025 = $this->iku->cnIndeks2025($dataIndeks['tahun'],$dataIndeks['uid_provinsi'],0,true);
                $cnIndeks["indeksIkuInaTitik"] = $cnIndeks2025["titik"];
                $cnIndeks["indeksIkuInaDetail"] = $cnIndeks2025["provinsi"];
                $cnIndeks["indeksIkuIna"] = $cnIndeks2025["provinsi"]["iku"];
                $cnIndeks["rataanIndeksIna"] = $cnIndeks2025["provinsi"]["indeks_rataan"];
                $cnIndeks["all"] = $cnIndeks2025;
    ...
    {// function get data indeks pelaporan
      $tahunShow = $this->params('x');
        ...
        // $sqlKabkota   = 'SELECT a.*, b.nama_propinsi AS nama_provinsi, c.nama_kabkot AS nama_kabkota FROM indeks_iku a LEFT JOIN rf_provinsi b ON b.kd_propinsi = a.uid_provinsi LEFT JOIN rf_kabkota c ON c.kd_kota = a.uid_kabkota WHERE a.jenis_indeks=0 AND ' . $w . ' ORDER BY ' . $o . ' LIMIT ' . $offset . ',' . $limit;
        // $sqlProv      = 'SELECT a.*, b.nama_propinsi AS nama_provinsi, c.nama_kabkot AS nama_kabkota FROM indeks_iku a LEFT JOIN rf_provinsi b ON b.kd_propinsi = a.uid_provinsi LEFT JOIN rf_kabkota c ON c.kd_kota = a.uid_kabkota WHERE a.jenis_indeks=1 AND ' . $w . ' ORDER BY ' . $o . ' LIMIT ' . $offset . ',' . $limit;
        ...
        // $this->debug->show($getDataNasional['data'][0]);
