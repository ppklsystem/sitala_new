 ...
 * desc : controller for peruntukan iklh
 */
class nilaipelengkapiklhController extends Front {
	public function init() {
		...
		$this -> view -> assign("primaryKey", "uid_nilai_pelengkap_iklh");
		$this -> viewName = "v_nilai_pelengkap_iklh";
		$this -> primaryKey = "uid_nilai_pelengkap_iklh";
	...
	public function index() {
		...
		if (isset($post['submit'])) {
			...
			$post['form']['iku_pm25_satelit'] = str_replace(",", '.', $post['form']['iku_pm25_satelit']);
			$post['form']['uid_kabkota'] = ($post['form']['peruntukan'] == 1 ? $post['form']['uid_kabkota'] : 0);
			if ($post['form']['uid_kabkota']) {
				$wTarget = " deleted = 0 AND uid_provinsi=".$post['form']['uid_provinsi']." AND uid_kabkota =".$post['form']['uid_kabkota']." AND peruntukan = 1 AND tahun=".$post['form']['tahun'];
			} else {
				$wTarget = " deleted = 0 AND uid_provinsi=".$post['form']['uid_provinsi']." AND peruntukan = 2 AND tahun=".$post['form']['tahun'];
			}
			if(!$post['form'][$this->primaryKey]){
				$cekTarget = $this->tables->query("SELECT * FROM rf_nilai_pelengkap_iklh WHERE ".$wTarget);
			}
			if($cekTarget['total']){
				$message = "Gagal menyimpan data, tahun ".$post['form']['tahun']." sudah memiliki peruntukan nilai IKLH, silahkan update data yang sudah ada jika ada perubahan !";
			}else{
		...
		if (isset($post['submit-excel'])) {
				...
				if ($files) {
						...
						foreach ($data as $key => $vals) {
								if ($vals[1] != "-") {
										$postTarget['form']['uid_nilai_pelengkap_iklh'] = "";
										$postTarget['form']['cruser'] = $post['form']['cruser'];
										if ($vals[4] == "KABUPATEN") {
											$postTarget['form']['peruntukan'] = 1;
										} else {
											$postTarget['form']['peruntukan'] = 2;
										}
										...
										$postTarget['form']['iku_pm25_satelit'] = str_replace(",", '.', $vals[5]);

										if ($postTarget['form']['uid_kabkota']) {
											$wTarget = " deleted = 0 AND uid_provinsi=".$postTarget['form']['uid_provinsi']." AND uid_kabkota =".$postTarget['form']['uid_kabkota']." AND peruntukan = 1 AND tahun=".$postTarget['form']['tahun'];
											// =(0,376*J4)+(0,405*I4)+(0,219*K4)
										} else {
											$wTarget = " deleted = 0 AND uid_provinsi=".$postTarget['form']['uid_provinsi']." AND peruntukan = 2 AND tahun=".$postTarget['form']['tahun'];
											// =(0,34*J3)+(0,428*I3)+(0,133*K3)+(0,099*L3)
										}

										$cek_tahun_peruntukan = $this -> tables -> query("SELECT * FROM rf_nilai_pelengkap_iklh WHERE ".$wTarget);
								...
								if ($tmpTahun) {
									$message = "Nilai Pelengkap provinsi ".implode(', ', $tmpTahun)." tidak tersimpan oleh sistem dikarenakan tahun tersebut sudah ada nilai. Selain dari tahun tersebut data berhasil disimpan";
									$message .= ($tmpProv ? "<br><br>Nilai Pelengkap provinsi ".implode(',',array_unique($tmpProv)). " tidak tersimpan karena provinsi tidak terdaftar disistem atau nama tidak sesuai dengan disistem" : "");
								}else {
									if (count($data) == $tmpCn) {
	...
	public function dataExcel($w=null, $offset=null)
	{
			...
			header('Content-Disposition: attachment; filename="NILAI_PELENGKAP_IKLH_'.time().'.xls"');
			$html = $this->view->fetch('parts/contents/nilaipelengkapiklh/index/excel.html');
			echo $html;
	}
	...
	public function editData() {
		...
		if ($this -> params("x")) {
			$this -> tables -> set("rf_nilai_pelengkap_iklh", "uid_nilai_pelengkap_iklh");
			$dataEdit = $this -> tables -> fetch("deleted = 0 AND uid_nilai_pelengkap_iklh=" . $this -> params("x"));
