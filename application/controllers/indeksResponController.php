...
class indeksResponController extends Front
{
    ...
    private function dataTable()
    {
      ...
      if ($this -> me['role_user'] == 3) {
          ...
          if($this->me["uid_provinsi_lainnya"]){
            ...
            $w .= " AND b.uid_provinsi IN ({$provinsiLainnya})";
          }
      }
      ...
      if($this->yearActive < 2023){
        if( $this->me['role_user']== 0 || $this->me['role_user']==4 || $this -> me['role_user'] == 5){
          $wKd = ' AND c.kd_propinsi < 34';
          // $wKd .= ' AND d.deleted = 1';
        }
      }
      if($this->me['role_user'] == 2){
        if($this->yearActive > 2022){
          // $wKd = ' AND d.deleted = 0';
        }
        if($this->yearActive < 2023){
            $wKd .= ' AND c.kd_propinsi < 34';
        }
        // $this->debug->shOW($this->me['role_user']);
      }
          // if ($this->me['role_user']==2) {
          //   $wKd = ' AND deleted = 0';
          // }elseif (condition) {
      ...
      $sql = 'SELECT MIN(a.indeks_uid) as min_index, GROUP_CONCAT(a.indeks_uid SEPARATOR ",") indeks_gabung, a.users_uid, a.tahun, MIN(a.tipe) as min_tipe, GROUP_CONCAT(a.tipe SEPARATOR ",") AS tipe_gabung,
              ...
              WHERE '. $w .$wKd.' GROUP BY a.users_uid ORDER BY a.tahun DESC LIMIT '. $offset .','. $limit;
              // $this->debug->show($sql);
      ...
      if($totalRow > 0){
        ...
        $LB = $LB->fields['x'];
        $LBKABKOT = $this -> db -> query('SELECT COUNT(id) as x FROM irf_langitbiru WHERE deleted=0 AND hidden=0 AND parent_3 > 0 AND prov = 0');
        // $this->debug->show($LBKABKOT);
        $LBKABKOT = $LBKABKOT->fields['x'];
        ...
        $PB = $PB->fields['x'];
        $PBKABKOT = $this -> db -> query('SELECT COUNT(id) as x FROM irf_pantaibersih WHERE deleted=0 AND hidden=0 AND parent_3 > 0 AND prov = 0');
        $PBKABKOT = $PBKABKOT->fields['x'];
        ...
        $PKB = $PKB->fields['x'];
        $PKBKABKOT = $this -> db -> query('SELECT COUNT(id) as x FROM irf_pkb WHERE deleted=0 AND hidden=0 AND parent_3 > 0 AND prov = 0');
        $PKBKABKOT = $PKBKABKOT->fields['x'];
        ...
        $IH = $IH->fields['x'];
        $IHKABKOT = $this -> db -> query('SELECT COUNT(id) as x FROM irf_indohijau WHERE deleted=0 AND hidden=0 AND parent_3 > 0 AND prov = 0');
        $IHKABKOT = $IHKABKOT->fields['x'];
        ...
        $GL = $GL->fields['x'];
        $GLKABKOT = $this -> db -> query('SELECT COUNT(id) as x FROM irf_gambut WHERE deleted=0 AND hidden=0 AND parent_3 > 0 AND prov = 0');
        $GLKABKOT = $GLKABKOT->fields['x'];

        $isian_per_tipe = array(1 => $LB, 2 => $PB, 3 => $PKB, 4 => $IH, 5 => $GL);
        $isian_per_tipe_kabkot = array(1 => $LBKABKOT, 2 => $PBKABKOT, 3 => $PKBKABKOT, 4 => $IHKABKOT, 5 => $GLKABKOT);
          ...
          foreach($child['data'] as $ke=>$va){
            if($val['tahun'] > 2022){
              if($val['uid_kabkota'] > 0){
                $total_isian = $isian_per_tipe_kabkot[$va['tipe']];
              }else{
            ...
            if ($isian[$va['tipe']]['persen_mengisi'] > 100) {
              // $isian[$va['tipe']]['persen_mengisi'] = 100;
            }
          }
    ...
    public function dataExcel($w, $offset)
  	{
        ...
        if($totalRow > 0){
          ...
          foreach($data['data'] as $key=>$val){
            ...
            foreach($child['data'] as $ke=>$va){
              ...
              // if ($isian[$va['tipe']]['persen_mengisi'] > 100) {
              //   $this->debug->show($isian_per_tipe_kabkot);
              //   $this->debug->show($va['jumlah_mengisi'].' - '.$total_isian);
              // }
            }
            $data['data'][$key]['isian'] = $isian;
          }
        ...
        header('Content-Disposition: attachment; filename="ISIAN_INDEKS_RESPON_'.time().'.xls"');
        $html = $this->view->fetch('parts/contents/indeksRespon/index/excel.html');
  			echo $html;
  	}
    ...
    public function respon()
    {
      ...
      if(isset($post['submit-data'])) {
        ...
        foreach ($post['form']['parent_uid'] as $key => $value) {
          ...
          // $file[$ref_uid] = $_FILES['bukti'];
          // $fileUpload = $this -> functions -> uploadFiles($_FILES['bukti'], "respon");
          // $filename[$ref_uid] = $fileUpload;
          ...
          foreach ($file[$ref_uid] as $ky => $val) {
            ...
            if (isset($tmpFilePath) && $tmpFilePath){
              ...
              $check_ext = $this->functions->validateExtension($file_upload);
              if ($check_ext) {
          ...
          if ($this -> tables -> post($post1)) {
            ...
            $this->view-> assign("collapse", $post['form']['collapse']);
          } else {
      ...
      if($post['form']['tahun'] > 2023){
        die("<br><br><br><br><br><br><center><h3 style='padding:0px;margin:0px;'>Penilaian Indeks Respon 2024 sudah dipindahkan ke aplikasi Indeks respon</h3></center>");
      }
        /*
          TIPE :
          1. LANGIT BIRU (LB)
    ...
    {
      if ($this->me['role_user'] == 3 || $this->roleDataView == 3) {
        $w = ' AND prov=0';
      }
        ...
        foreach ($data['data'] as $key => $value) {
            ...
            foreach ($data2['data'] as $ky => $val) {
                ...
                foreach ($data3['data'] as $k => $v) {
                    ...
                    $data['data'][$key]['child3Total'] = $data['data'][$key]['child1'][$ky]['child2'][$k]['tchild3'] ? 1:0;
                }
            }
