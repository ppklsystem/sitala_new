 ...
 */
class refDasController extends Front
{
  ...
  public function index(){
    ...
    if(isset($post['submit'])){
      ...
      $namaDas = $post["form"]["nama"];
      $this->tables->set("rf_das","uid");
      $checkNama = $this->tables->fetch("deleted = 0 AND nama LIKE '{$namaDas}'");
      if($checkNama["total"] > 0){
        $message = "Gagal menimpan data, das {$namaDas} sudah terdaftar !";
      }else{
    ...
    $this->view->assign("title",'Nama Das');
      ...
      if(isset($post['search'])){
        ...
        if($post['form']['keyword']){
          if($properties['total']){
			...
			header('Content-Disposition: attachment; filename="DAS_IKLH_'.time().'.xls"');
			$html = $this->view->fetch('parts/contents/refDas/index/excel.html');
			echo $html;
	}
