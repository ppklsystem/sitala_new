...
class ikaController extends Front
{
            ...
            if ($files) {
                ...
                foreach ($data as $key => $vals) {
                    if ($vals[1] != "-") {
                        ...
                        $postLaporan['form']['kategori'] = $this -> kategori($vals[6]);
                        $postLaporan['form']['ph'] = str_replace(",", ".", $vals[7]);
                        $postLaporan['form']['bod'] = str_replace(",", ".", $vals[8]);
                        $postLaporan['form']['cod'] = str_replace(",", ".", $vals[9]);
                        $postLaporan['form']['tss'] = str_replace(",", ".", $vals[10]);
                        $postLaporan['form']['do_p'] = str_replace(",", ".", $vals[11]);
                        //$postLaporan['form']['do_max_p'] = str_replace(",", ".", $vals[12]);
    ...
    {
        $numList 	= round($totalRow/500);
        $numListRes = $numList * 500;
        ...
        for ($itemCount; $itemCount<=$numList; $itemCount++) {
            if ($itemCount == 1) {
                ...
                $listExport[$itemCount]['offset_start'] = ($limitCount - 500) + 1;
            }
    ...
    public function expExcel($kabkota = null, $provinsi = null)
    {
        ...
        if ($kabkota) {
            ...
            header("Content-type: application/vnd-ms-excel");
            header('Content-Disposition: attachment; filename="PELAPORAN_IKA_KABKOTA'.time().'.xls"');
            $html = $this->view->fetch('parts/contents/ika/indeks/excel_kabkota.html');
            echo $html;
        } elseif ($provinsi) {
            $this -> view -> assign("viewp", $provinsi['data']);
            header("Content-type: application/vnd-ms-excel");
            header('Content-Disposition: attachment; filename="PELAPORAN_IKA_PROVINSI'.time().'.xls"');
    ...
    public function _countIndekStatusMutu($idIndeks)
    {
        ...
        $cekData = $this -> tables -> query("SELECT a.uid_pelaporan_ika, YEAR(a.tanggal) AS tahun, a.uid_lokasi_pemantauan FROM pelaporan_ika_wqms a WHERE " . $whereIndeksParams);
        if ($cekData['total']) {
            $dataKelas = null;
            $sqlKelas = null;
            for ($i = 1; $i <= 4; $i++) {
    ...
    public function getCatatanVerifikasi()
    {
        $uid = $this->params("x");
        if (is_numeric($uid)) {
            $data = $this->tables->query("SELECT catatan_verifikator AS catatan, catatan_provinsi FROM ".$this->viewName." WHERE ".$this->primaryKey." = ".$uid);
            if($this->me['role_user'] == 2){
              $catatan = $data['data'][0]['catatan_provinsi'];
            }else{
              $catatan = $data['data'][0]['catatan'];
            }
            echo json_encode(array("statusCode"=>200, "catatan"=>$catatan));
        } else {
    ...
    {// function for generate field indeks
        $cekDataKabkota = $this -> tables -> query("SELECT uid_indeks_ika FROM indeks_ika_wqms WHERE deleted = 0 AND jenis_indeks =0  AND uid_kabkota=" . $uid_kabota . " AND tahun=" . $tahun);
        if (!$cekDataKabkota['total']) {
            $this -> tables -> set("indeks_ika_wqms", "uid_indeks_ika");
        ...
        $cekDataProvinsi = $this -> tables -> query("SELECT uid_indeks_ika FROM indeks_ika_wqms WHERE deleted = 0 AND jenis_indeks =1 AND uid_provinsi=" . $uid_provinsi . " AND tahun=" . $tahun);
        ...
        $cekDataNasional = $this -> tables -> query("SELECT uid_indeks_ika FROM indeks_ika_wqms WHERE deleted = 0 AND jenis_indeks =2 AND tahun=" . $tahun);
